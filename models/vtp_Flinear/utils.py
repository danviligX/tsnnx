...
import optuna
import pickle
from libx.dataio import Args,vtp_dataloader,Dset
...
class vtp_Flinear(nn.Module):
    def __init__(self,args) -> None:
        ...
        self.flinear = nnx.FLinear(in_feature=self.his_len,out_feature=self.pre_len,basis_num=self.basis_num)
...
def obj_vtp_Flinear(trial):
    ...
    args.embadding_size = trial.suggest_int("embadding_size", 8, 1024,step=8)
    ...
    args.batch_size = trial.suggest_int("batch_size", 4, 256,step=4)
    ...
    for epoch in range(args.epoch_num):
        ...
        epoch_error,_ = valid(net,valid_loader,criterion,data,args)
    ...
    torch.save(net.state_dict(),''.join(['./models/',args.model_name,'/trial/trial_',str(trial.number),'.mdic']))
    torch.save(args,''.join(['./models/',args.model_name,'/trial/args_',str(trial.number),'.marg']))
    return optuna_error
